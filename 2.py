...
with col2:
    ...
    if st.session_state['scene_cabinets']:
        ...
        try:
            ...
            dxf_dir_exists = os.path.isdir(os.path.join(APP_DIR, "dxf_export"))
            st.error(
                f"Module introuvable: {exc}. "
                f"dxf_export présent dans le repo déployé: {dxf_dir_exists}. "
                "Si False: pousse le dossier dxf_export (avec __init__.py). "
                "Si True: redéploie après Reboot/Clear cache Streamlit Cloud."
            )
            st.stop()
        except Exception as exc:
            st.error(f"Erreur au chargement des modules d'export DXF: {exc}")
            st.stop()
